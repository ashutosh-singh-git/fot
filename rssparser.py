...
for post in posts:
    ...
    latest_date = parser.parse(post.published) if latest_date != null
    # if any post doesn't have information then throw error.
    try:
        #Sat, 29 Oct 2022 18:45:21 +0530
        # date_time_obj = datetime. strptime(post.published, '%d/%m/%y %H:%M:%S')
        latest_date = parser.parse(post.published)
